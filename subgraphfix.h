#ifndef BOOSTSUBGRAPHFIX_H
#define BOOSTSUBGRAPHFIX_H
...
/*
 *  this file is for fixing some boost shit = =||
*/
...
namespace boost
{
    ...
    public:
        ...
        vertex_descriptor local_to_global(vertex_descriptor u_local) const
        {
            ...
            {
                BOOST_ASSERT(m_global_vertex.size() > u_local);
        ...
        edge_descriptor local_to_global(edge_descriptor e_local) const
        {
            ...
            {
                edge_index_type e_global_idx = get(get(edge_index, m_graph), e_local);
                BOOST_ASSERT(m_global_edge.size() > e_global_idx);
                return m_global_edge[e_global_idx];
            }
        ...
        edge_descriptor global_to_local(edge_descriptor e_global) const
        {
            ...
            {
                edge_index_type e_local_idx = get(get(edge_index, root().m_graph), e_global);
                typename std::map<edge_index_type, edge_descriptor>::const_iterator
                    found = m_local_edge.find(e_local_idx);
                BOOST_ASSERT(found != m_local_edge.end());
                return found->second;
            }
    ...
    private:
        ...
        typedef std::map< edge_descriptor, edge_descriptor > LocalEdgeMap;
                      ...
                      subgraph< Graph >* orig)
    {
        if (g.is_root())
        {
            ...
            g.m_global_edge.push_back(e_global);  // is this a copy????
            children_add_edge(u_global, v_global, e_global, g.m_children, orig);
    ...
    namespace detail
    {
            ...
            Vertex u_global, Vertex v_global, subgraph< Graph >& g)
        {
            ...
            {
                std::pair<typename subgraph<Graph>::edge_descriptor, bool>
                    e_desc = edge(u_global, v_global, g);
                if (e_desc.second)
                    remove_edge(e_desc.first, g);
            }
            else
            {
        ...
        void remove_edge_recur_down(
            Edge e_global, subgraph< Graph >& g)
        {
            std::pair<typename subgraph< Graph >::edge_descriptor, bool> e_local = g.find_edge(e_global);
            if (!e_local.second)
                return;

            subgraph<Graph> root = g.root();
            typename subgraph<Graph>::edge_index_type e_idx = get(get(edge_index, root.m_graph), e_global);

            typename std::vector<typename subgraph<Graph>::edge_descriptor>::const_iterator
                found = std::find_if(g.m_global_edge.begin(), g.m_global_edge.end(), [&root, &e_idx](const typename subgraph<Graph>::edge_descriptor& edge_desc){
                      return get(get(edge_index, root.m_graph), edge_desc) == e_idx;
                });

            if (found != g.m_global_edge.end())
            {
                g.m_global_edge.erase(found);  // std::vector< edge_descriptor >
                g.m_local_edge.erase(e_idx);  // std::map< edge_index_type, edge_descriptor >
                // boost originally does not change the m_edge_counter, which is for edge indexing
                g.m_edge_counter--;  // now indexing is broken, need to update it
            }

            typedef indirect_iterator< typename std::list< subgraph< Graph >* >::const_iterator,
                                      subgraph< Graph >, std::bidirectional_iterator_tag >
                children_iterator;
            typedef indirect_iterator< typename std::list< subgraph< Graph >* >::const_iterator,
                                      subgraph< Graph > const, std::bidirectional_iterator_tag >
            ...
            std::pair< const_children_iterator, const_children_iterator > children = g.children();
            for (children_iterator child = children.first; child != children.second; child++)
                remove_edge_recur_down(e_global, *child);

            remove_edge(e_local.first, g.m_graph);
        }

        // [REFLEX]
        template <typename G>
        void children_edge_reindex(const typename subgraph<G>::edge_index_type& global_idx, boost::subgraph<G>& g)
        {
            // we don't need to update m_global_edge I guess
            typename subgraph<G>::edge_index_type e_global_idx;
            typedef std::map<typename subgraph<G>::edge_index_type,
                             typename subgraph<G>::edge_descriptor> local_map;

            typename subgraph<G>::edge_index_type e_local_idx = 0;
            std::set<typename subgraph<G>::edge_index_type> to_remove;
            for (typename local_map::iterator it = g.m_local_edge.begin(); it != g.m_local_edge.end(); it++)
            {
                if (it->first > global_idx)
                {
                    put(edge_index, g.m_graph, g.m_local_edge[it->first],
                        (typename subgraph<G>::edge_index_type)(e_local_idx));
                    g.m_local_edge[it->first - 1] = g.m_local_edge[it->first];

                    to_remove.erase(it->first - 1);
                    to_remove.insert(it->first);
                }
                e_local_idx++;
            }
            for (const typename subgraph<G>::edge_index_type&
                 idx_to_remove: to_remove)
                g.m_local_edge.erase(idx_to_remove);

            typedef indirect_iterator<typename std::list< subgraph<G>*>::const_iterator,
                                      subgraph<G>, std::bidirectional_iterator_tag>
                children_iterator;
            typedef indirect_iterator<typename std::list< subgraph<G>*>::const_iterator,
                                      subgraph<G> const, std::bidirectional_iterator_tag>
                const_children_iterator;

            std::pair<const_children_iterator, const_children_iterator>
                children = g.children();
            for (children_iterator it = children.first; it != children.second; it++)
                children_edge_reindex(global_idx, *it);
        }


        // [REFLEX]
        template <typename G>
        void edge_reindex(const typename subgraph<G>::edge_descriptor& e_global, boost::subgraph<G>& g)
        {
            if (!g.is_root())
            {
                edge_reindex(e_global, g.root());
                return;
            }

            const typename subgraph<G>::edge_index_type&
                e_global_idx = get(get(edge_index, g.m_graph), e_global);

            // this solution is easy to understand but slowly
            std::map<typename subgraph<G>::edge_index_type, typename subgraph<G>::edge_index_type> m;
            for (size_t idx = e_global_idx; idx < g.m_global_edge.size(); idx++)
                put(edge_index, g.m_graph, g.m_global_edge[idx], (typename subgraph<G>::edge_index_type)idx);
            ...
            std::pair<const_children_iterator, const_children_iterator>
                children = g.children();
            for (children_iterator it = children.first; it != children.second; it++)
                children_edge_reindex(e_global_idx, *it);
        }
    ...
    void remove_edge(typename subgraph< G >::edge_descriptor e, subgraph< G >& g)
    {
        if (g.is_root())
        {
            detail::remove_edge_recur_down(e, g);
            detail::edge_reindex(e, g);
        }
        else
        {
            remove_edge(g.local_to_global(e), g.root());
        }
    }
    ...
    void remove_edge(typename subgraph< G >::vertex_descriptor u,
                     typename subgraph< G >::vertex_descriptor v, subgraph< G >& g)
    {
        // the loop is for parallel edges
        // not sure if need a loop 0_0
        // but my test says yes O_O
        while (true)
        {
            std::pair<typename subgraph< G >::edge_descriptor, bool>
                edge_desc = edge(u, v, g);
            if (edge_desc.second)
                remove_edge(edge_desc.first, g);
            else
                break;
        }
    }
    ...
    void remove_edge_if(Predicate p, subgraph< G >& g)
    {
        ...
        std::vector<typename subgraph<G>::edge_descriptor> to_remove;
        std::pair<ei_type, ei_type> ep = edges(g);
        for (auto it = ep.first; it != ep.second; it++)
        {
            if (p(*it))
                to_remove.push_back(*it);  // not sure if a copy still works
        }

        for (typename subgraph<G>::edge_descriptor& desc: to_remove)
            remove_edge(desc, g);
    }
    ...
    void clear_vertex(typename subgraph< G >::vertex_descriptor v, subgraph< G >& g)
    {
        while (true)
        {
            typedef typename subgraph< G >::out_edge_iterator out_edge;
            std::pair< out_edge, out_edge > p_out = out_edges(v, g.m_graph);
            if (p_out.first == p_out.second)
                break;
            remove_edge(*p_out.first, g);
        }

        // [REFLEX]
        while (true)
        {
            typedef typename subgraph< G >::in_edge_iterator in_edge;
            std::pair< in_edge, in_edge > p_in = in_edges(v, g.m_graph);
            if (p_in.first == p_in.second)
                break;
            remove_edge(*p_in.first, g);
        }
    }
    ...
    namespace detail
    {
        // [REFLEX]
        /**
         * @brief update_index_in_edge
         *  call this function before remove the vertex!
         * @param u_global
         * @param g
         */
        template <typename G>
        void update_index_in_edge(const typename subgraph<G>::vertex_descriptor& u_global, subgraph<G>& g)
        {
            for (typename std::vector<typename subgraph<G>::edge_descriptor>::iterator
                     it = g.m_global_edge.begin(); it != g.m_global_edge.end(); it++)
            {
                if (it->m_source > u_global)
                    it->m_source -= 1;
                if (it->m_target > u_global)
                    it->m_target -= 1;
            }

            typename subgraph<G>::vertex_descriptor u_local;
            bool found;
            boost::tie(u_local, found) = g.find_vertex(u_global);
            if (found)
            {
                for (typename std::map<typename subgraph<G>::edge_index_type,
                                       typename subgraph<G>::edge_descriptor>::iterator
                         it = g.m_local_edge.begin(); it != g.m_local_edge.end(); it++)
                {
                    if (it->second.m_source > u_local)
                    it->second.m_source -= 1;
                    if (it->second.m_target > u_local)
                    it->second.m_target -= 1;
                }
            }
            ...
            std::pair<const_children_iterator, const_children_iterator>
                children = g.children();
            for (children_iterator it = children.first; it != children.second; it++)
                update_index_in_edge(u_global, *it);
        }


        // [REFLEX]
        template <typename G>
        void remove_vertex_recur_down(typename subgraph<G>::vertex_descriptor u_global, subgraph<G>& g)
        {
            std::pair<typename subgraph<G>::vertex_descriptor, bool >
                fu = g.find_vertex(u_global);
            if (fu.second)
            {
                remove_vertex(fu.first, g.m_graph);
                g.m_local_vertex.erase(u_global);
                auto f = std::find(g.m_global_vertex.begin(), g.m_global_vertex.end(), u_global);
                BOOST_ASSERT(f != g.m_global_vertex.end());
                g.m_global_vertex.erase(f);
                ...
                std::pair<const_children_iterator, const_children_iterator>
                    children = g.children();
                for (children_iterator it = children.first; it != children.second; it++)
                    remove_vertex_recur_down(u_global, *it);
            }
        }
        ...
        // [REFLEX]
        template <typename G>
        void children_vertex_reindex(const typename subgraph<G>::vertex_descriptor& u_global, subgraph<G>& g)
        {
            std::set<typename subgraph<G>::edge_index_type> to_remove;
            for (size_t idx = 0; idx < g.m_global_vertex.size(); idx++)
            {
                if (g.m_global_vertex[idx] > u_global)
                {
                    g.m_global_vertex[idx] -= 1;

                    typename subgraph<G>::vertex_descriptor
                        u_global_idx = g.m_global_vertex[idx];
                    g.m_local_vertex[u_global_idx] = (typename subgraph<G>::vertex_descriptor)idx;

                    to_remove.erase(u_global_idx);
                    to_remove.insert(u_global_idx + 1);
                }
            }

            for (const typename subgraph<G>::edge_index_type&
                     idx_to_remove: to_remove)
                g.m_local_vertex.erase(idx_to_remove);
            ...
            std::pair<const_children_iterator, const_children_iterator>
                children = g.children();
            for (children_iterator it = children.first; it != children.second; it++)
                children_vertex_reindex(u_global, *it);
        }


        // [REFLEX]
        template <typename G>
        void vertex_reindex(const typename subgraph<G>::vertex_descriptor& u_global, subgraph<G>& g)
        {
            if (!g.is_root())
            {
                vertex_reindex(u_global, g.root());
                return;
            }

            for (size_t idx = u_global; idx < g.m_global_vertex.size(); idx++)
                g.m_global_vertex[idx] -= 1;
    ...
    // [REFLEX]
    template <typename G>
    void remove_vertex(typename subgraph<G>::vertex_descriptor u, boost::subgraph<G>& g)
    {
        if (g.is_root())
        {
            boost::clear_vertex(u, g);  // cleanup vertex
            detail::update_index_in_edge(u, g);
            detail::remove_vertex_recur_down< G >(u, g);
            detail::vertex_reindex(u, g);
        }
        else
        {
            typename boost::subgraph< G >::vertex_descriptor u_global = g.local_to_global(u);
            boost::subgraph< G >& root = g.root(); // to root
            remove_vertex(u_global, root);
        }
    }
 ...
 */
    ...
    namespace detail
    {
        // [REFLEX]
        template <typename G>
        void clear_graph_recur_down(boost::subgraph<G>& g)
        {
            // cleanup all the containers.
            g.m_graph.clear();
            g.m_edge_counter = 0;
            g.m_global_edge.clear();
            g.m_local_edge.clear();
            g.m_global_vertex.clear();
            g.m_local_vertex.clear();

            typedef std::list<boost::subgraph<G>*> ChildrenList;
            typedef boost::indirect_iterator<typename ChildrenList::const_iterator,
                                             boost::subgraph<G> const,
    ...
    // [REFLEX]
    template <typename G>
    void clear_graph(subgraph<G>& g)
    {
        if (!g.is_root())
            clear_graph(g.root());
        else
            detail::clear_graph_recur_down(g);
    }

} // namespace boost

#endif // BOOSTSUBGRAPHFIX_H
